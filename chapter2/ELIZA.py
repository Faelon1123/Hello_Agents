...
rules = {
    r'I need (.*)': [
        "Why do you need {0}?", # 格式化模板，0表示占位符，表示使用.format()中的第0个参数
...
def respond(user_input):
    ...
    for pattern, responses in rules.items():
        match = re.search(pattern, user_input, re.IGNORECASE) # 使用正则表达式匹配，忽略大小写
        if match:
            # 捕获匹配到的部分
...
# Therapist: Would it really help you to get some help with your project.?
